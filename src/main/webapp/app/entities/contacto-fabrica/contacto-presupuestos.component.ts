...
@Component({
    ...
    templateUrl: './contacto-presupuestos.component.html'
})
export class ContactoPresupuestosComponent implements OnInit, OnDestroy {
    currentAccount: any;
            ...
            .subscribe(
                (res: HttpResponse<IProductosPresupuestoPedidos[]>) => {
                    for (let i = 0; i < res.body.length; i++) {
                        productosPed[i] = res.body[i];
                    }
                    this.productosPedidos = productosPed;
                },
    ...
    public cargarProd() {
        var id = $('#modalIncidencias #opciones').val();
        var prod = this.productosPedidos;
        for (let k = 0; k < prod.length; k++) {
            if (prod[k]['presupuestoPedido']['id'] == id) {
                $('#modalIncidencias #opcionesProducto').append(
                    '<option value="' + prod[k]['id'] + '">' + prod[k]['productosDormitorio']['nombre'] + '</option>'
                );
            }
        }
    }
    ...
    public llenarModal(id) {
        ...
        if (id == 3) {
            $('#modalIncidencias #relacion').val('Incidencias');
            for (let i = 0; i < pedidos.length; i++) {
                if (usuario['id'] == pedidos[i]['user']['id'] && pedidos[i]['pedido'] == 1) {
                    $('#modalIncidencias #opciones').append(
                        '<option value="' + pedidos[i]['id'] + '">' + pedidos[i]['codigo'] + '</option>'
                    );
                }
            }
        }
    ...
    public crearChat(id) {
        ...
        if (id == 2) {
            var tipo = $('#modalIncidencias #relacion').val();
            var prodId = $('#modalIncidencias #opcionesProducto').val();
            var numero;
            var ProductoBueno;
            var idPed;
            idPed = $('#modalIncidencias #opciones').val();
            var prod = this.productosPedidos;
            for (let k = 0; k < prod.length; k++) {
                if (prod[k]['id'] == prodId) {
                    ProductoBueno = prod[k];
                }
            }
        }
        ...
        if (tipo == 'Incidencias') {
            ...
            this.contactoFabricaService
                ...
                .subscribe(
                    (res: HttpResponse<IContactoFabrica[]>) => {
                        for (let i = 0; i < res.body.length; i++) {
                            if (aux.length == 0 || aux[0]['id'] < res.body[i]['id']) {
                                aux[0] = res.body[i];
                            }
                        }
                        const mensaje = {
                            texto: 'INC-PROD-CR-TG',
                            productosPresupuestoPedidos: ProductoBueno,
                            contactoFabrica: aux[0],
                            user: this.currentAccount
                        };
                        this.aux = aux;

                        this.subscribeToSaveResponse1(this.mensajesService.create(mensaje));
                        if (aux.length != 0) {
                            this.onSaveSuccess1();
                        }
                    },
    ...
    protected subscribeToSaveResponse(result: Observable<HttpResponse<IContactoFabrica>>) {
        result.subscribe((res: HttpResponse<IContactoFabrica>) => this.onSaveSuccess1(), (res: HttpErrorResponse) => this.onSaveError1());
    }
    ...
    protected subscribeToSaveResponse1(result: Observable<HttpResponse<IMensajes>>) {
    ...
    protected onSaveSuccess1() {
        this.isSaving = false;
        var aux = this.aux;
        this.router.navigate(['/contacto-fabrica', aux[0]['id'], 'chat']);
    }

    protected onSaveError1() {
        this.isSaving = false;
    }
}
