...
export class ProductosDormitorioApoyoComponent implements OnInit, OnDestroy {
    ...
    public open(producto, productoNombre) {
        ...
        $('.productosColor17').css({ 'background-color': 'white' });
        $('.productosColor18').css({ 'background-color': 'white' });
        $('.productosColor16').css({ 'background-color': 'white' });
        $('.productosColor15').css({ 'background-color': 'white' });
        ...
        this.nombreApoyo = productoNombre;
        if (productoNombre == 'Bancada' || productoNombre == 'Patas Metalicas') {
            this.esBancadaOPatas = 1;
        } else {
            this.esBancadaOPatas = 0;
        }
        if (productoNombre == 'Zocalo') {
            $('#bancadaZocalo').css({ display: 'block' });
            $('#bancadaZocalo').css({ 'margin-top': '-1%' });
        ...
        if (productoNombre == 'Patas Regulables') {
            $('#patasRegulable').css({ display: 'block' });
            $('#patasRegulable').css({ 'margin-top': '-1%' });
            this.dimensionesProductoTipoService.findProducto(producto).subscribe(data => {
                this.dimensionesProductoTipoService.todos = data.body;
            });
        }
    ...
    public cogerFondo(id, fondo) {
        var idMueble;
        idMueble = $('#nombreMesita').attr('class');
        var nombre = $('#nombreMesita').text();
        if (nombre == 'Bancada' || nombre == 'Zocalo') {
            $('#bancadaZocalo').css({ display: 'block' });
        }
        if (nombre == 'Patas Metalicas') {
            $('#patasmetalicas').css({ display: 'block' });
            fondo = fondo - 1;
        }

        this.dimensionesProductoTipoService.findFiltroProd(idMueble, fondo).subscribe(data => {
            this.dimensionesProductoTipoService.todos = data.body;
        });
    }

    public cogidoAncho(ancho) {
        var val = $('#selectAnchoMedida').val();
        var todosDatos = this.dimensionesProductoTipoService.todos;
        var bueno;
        var nombre = $('#nombreMesita').text();
        if (nombre == 'Bancada') {
            for (let i = 0; i < todosDatos['length']; i++) {
                if (todosDatos[i][1] == val) {
                    bueno = todosDatos[i];
                }
            }
            var precioPunto = this.precioPunto[0];
            $('#precioDimension').text((bueno[3] * precioPunto).toFixed(2));
            $('#datos1').append(
                '<p style="width:100%"><span>Ancho : </span><span class="' + bueno[5] + '" id="ancho1">' + bueno[1] + '</span></p>'
            );
            $('#datos1').append('<p style="width:100%"><span>Alto : </span><span>' + bueno[0] + '</span></p>');
            $('#datos1').append('<p style="width:100%"><span>Fondo : </span><span>' + bueno[2] + '</span></p>');
        }
        if (nombre == 'Zocalo') {
            for (let i = 0; i < todosDatos['length']; i++) {
                if (todosDatos[i]['ancho'] == val) {
                    bueno = todosDatos[i];
                }
            }
            var precioPunto = this.precioPunto[0];
            $('#precioDimension').text((bueno['precio'] * precioPunto).toFixed(2));
            $('#datos1').append(
                '<p style="width:100%"><span>Ancho : </span><span class="' + bueno['id'] + '" id="ancho1">' + bueno['ancho'] + '</span></p>'
            );
            $('#datos1').append('<p style="width:100%"><span>Alto : </span><span>' + bueno['alto'] + '</span></p>');
            $('#datos1').append('<p style="width:100%"><span>Fondo : </span><span>' + bueno['fondo'] + '</span></p>');
        }
        if (nombre == 'Patas Metalicas') {
            for (let i = 0; i < todosDatos['length']; i++) {
                if (todosDatos[i][1] == ancho) {
                    bueno = todosDatos[i];
                }
            }
            var precioPunto = this.precioPunto[0];
        ...
        if (nombre == 'Patas Regulables') {
            for (let i = 0; i < todosDatos['length']; i++) {
                if (todosDatos[i]['ancho'] == ancho) {
                    bueno = todosDatos[i];
                }
            }
            var precioPunto = this.precioPunto[0];
