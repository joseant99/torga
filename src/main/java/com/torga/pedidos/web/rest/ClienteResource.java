...
public class ClienteResource {
    ...
    /**
     * GET  /cliente/representante/:id : get  all cliente with the "representante_id".
     *
     * @param representante_id of the cliente 
     * @return the ResponseEntity with status 200 (OK) and with body the cliente, or with status 404 (Not Found)
     */
    @GetMapping("/cliente/represenante/{id}")
    @Timed
    public ResponseEntity<List<Cliente>> getListClienteByRepresentanteId(@PathVariable Long id) {
        log.debug("REST request to get  List Cliente by representante_id : {}", id);
        List<Cliente> cliente = clienteService.getListClienteRepresentanteId(id);
        return ResponseEntity.ok()
                .body(cliente);
    }
    
    /**
     * GET  /cliente/:usuario : get  all cliente with the "usuario".
     *
     * @param user of the cliente 
     * @return the ResponseEntity with status 200 (OK) and with body the cliente, or with status 404 (Not Found)
     */
    @GetMapping("/cliente/{usuario}")
    @Timed
    public ResponseEntity<Cliente> getClienteByUser(@PathVariable String usuario) {
        log.debug("REST request to get Cliente by user : {}", usuario);
        Cliente cliente = clienteService.getClienteUsuario(usuario);
        return ResponseEntity.ok()
                .body(cliente);
    }
