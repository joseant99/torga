...
public interface MedidasEspecialesRepository extends JpaRepository<MedidasEspeciales, Long> {
	@Query("Select u from MedidasEspeciales u where u.productosDormitorio.id = ?1 and u.ancho = 1 and u.altura = ?2 order by u.id")
	Collection<MedidasEspeciales> findByProd(Long id,Float altura);
	@Query("Select u from MedidasEspeciales u where u.productosDormitorio.id = ?1 and u.alto = 1 and u.altura = ?2 order by u.id")
